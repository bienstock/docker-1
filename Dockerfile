...
RUN apt-get update && apt-get install -y git curl vim sudo && rm -rf /var/lib/apt/lists/*
...
RUN curl -fsSL ${JENKINS_URL} -o /usr/share/jenkins/jenkins.war \
  && echo "$(curl -s http://mirrors.jenkins.io/war-stable/${JENKINS_VERSION}/jenkins.war.sha256 | cut -d' ' -f1)  /usr/share/jenkins/jenkins.war" | sha256sum -c -
